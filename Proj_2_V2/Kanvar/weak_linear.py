...
class WeightedDecisionStump:
    ...
    def fit(self,X,y,sample_weight):
        ...
        for j in range(2):
            ...
            if np.allclose(x_sorted,x_sorted[0]):
                #all plus one
                pred_pos=np.ones_like(y_sorted)
                err_pos=np.sum(w_sorted[pred_pos!=y_sorted])

                #all minus one
                pred_neg=-np.ones_like(y_sorted)
                err_neg=np.sum(w_sorted[pred_neg!=y_sorted])

                if err_pos<best_error:
                    best_error=err_pos
                    best_feature=j
                    best_theta=x_sorted[0]
                    best_polarity=1
            ...
            #candidate thresholds midway between sorted points
            candidates=(x_sorted[:-1]+x_sorted[1:])/2.0

            for theta in candidates:
                #case 1 right side +1 left side -1
                pred_plus=np.where(xj>=theta,1,-1)
                err_plus=np.sum(w[pred_plus!=y])
                ...
                pred_minus=np.where(xj<=theta,1,-1)
                err_minus=np.sum(w[pred_minus!=y])
    ...
    def predict(self,X):
        ...
        if self.polarity==1:
            #right side is +1
            return np.where(xj>=self.threshold,1,-1).astype(int)
        else:
            #left side is +1
            return np.where(xj<=self.threshold,1,-1).astype(int)
